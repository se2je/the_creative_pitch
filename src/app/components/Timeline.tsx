      ...
      <section id="register" className="bg-black py-24 relative overflow-hidden border-t border-white/10">
        <div className="container mx-auto px-6 max-w-7xl relative z-10">
          ...
          <div className="mb-20 text-center">
            ...
            >
              {t('timeline.heading')} <span className="text-lime-500">{t('timeline.headingAccent')}</span> {t('timeline.headingEnd')}
            </motion.h2>
            ...
            <div className="flex justify-between relative z-10 max-w-4xl mx-auto">
              {timelineEvents.map((ev, i) => (
                  ...
                  >
                    ...
                    <div className="text-center bg-black/80 backdrop-blur-sm p-2 border border-white/10 rounded-sm group-hover:border-white/30 transition-colors">
                  <span
                  ...
                  >
                    {ev.date}
                      ...
                      <span className="text-xs uppercase tracking-widest text-gray-400 font-mono">{ev.title}</span>
                    </div>
                  </motion.div>
              ))}
            </div>
            ...
            {regOptions.map((opt, i) => (
                ...
                >
                  ...
                  <div className="flex items-start justify-between mb-8">
                    <opt.icon className={cn("w-12 h-12 stroke-1", opt.color === 'lime' ? 'text-lime-400' : 'text-purple-400')} />
                    <span className="font-mono text-xs border border-white/20 px-2 py-1 rounded text-gray-400 uppercase tracking-wider">
                  Option 0{i + 1}
                </span>
                  </div>
                  ...
                  <div className="space-y-4 mb-8">
                    <div className="flex items-center gap-3">
                      <div className={cn("w-1 h-1 rounded-full", opt.color === 'lime' ? 'bg-lime-500' : 'bg-purple-500')} />
                      <span className="text-sm text-gray-300 font-sans">{opt.req}</span>
                    </div>
                  </div>

                  <button className={cn(
                      "w-full py-4 font-bold font-mono uppercase tracking-widest text-sm transition-all hover:translate-y-[-2px]",
                      opt.color === 'lime'
