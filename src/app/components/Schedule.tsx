...
import { CheckCircle, Zap, Coffee, User, Flag, Trophy } from "lucide-react";
            ...
            {scheduleData.map((item, i) => (
                ...
                ${item.type === 'pitch' ? 'bg-lime-500 shadow-[0_0_15px_rgba(132,204,22,0.8)]' :
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-[140px_1fr] gap-4 md:gap-8 items-start group">
                    {/* Time */}
                    <div className="font-mono text-lime-500/80 text-sm py-1 border border-lime-500/20 bg-lime-900/10 px-2 rounded w-fit md:w-full text-center group-hover:bg-lime-500/20 transition-colors">
                      {item.time}
                    </div>
